...
| `planning` | Ferment exists; all phases in `"planned"` status (no `activate_ferment_phase` yet) | Research set + ferment planning tools: `read`, `grep`, `find`, `ls`, `web_fetch`, `web_search`, `propose_ferment_scoping`, `scope_ferment`, `update_ferment_scope_field`, `confirm_ferment_completion_criteria`, `list_ferments`, `ask_user` |
