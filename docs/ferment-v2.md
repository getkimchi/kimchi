...
With V2 enabled, approving a plan with **Execute** starts the same persistent controller and switches Plan to Auto. The initial objective keeps the exact approved Markdown; the saved plan is a reference copy, so changing or deleting that copy does not change the approved requirements. With V2 disabled, approval retains the legacy execution path.

The footer displays `◈ Plan execution: running · <name>` for approved plans and `◈ Ferment V2: running · <name>` for direct objectives. Each run gets a persisted name from the plan heading or objective, limited to six words and 36 display columns without another model request. Editing the objective refreshes its name; older sessions derive one on replay. `/ferment-v2` shows the approved plan's path without repeating the full Markdown. The footer changes to `checking` during evaluation and shows paused, blocked, complete, or budget-limited state. Narrow terminals shorten it to the state. There is no extra plan title or gradient in the prompt editor.

An explicit edit of a plan-derived run saves the full revised instructions to a fresh file under `.kimchi/plans` and stores only a read-file instruction. Reopening the editor loads that file, and the tool-free completion evaluator reads its requirements before evaluation. Earlier files remain untouched. Unlike the initial approval's reference copy, the edited file is authoritative and must remain available. Ordinary manual objectives remain inline.

The existing optional Plannotator adapter feeds the same approval path, so browser approval and TUI **Execute** both start V2 when enabled. The adapter forwards plan-review requests to the external extension and maps its approved/rejected result back to the shared decision handler; rejection requests rework or passes feedback. Browser results must match the current review ID and source, and adapter subscriptions are cleared at session boundaries to reject stale approvals. The first decision from either surface wins. This does not install Plannotator: without the external extension, use the TUI review popup. The browser adapter is inactive in headless, oneshot and subagent sessions.
...
The extension emits the full V2 observability contract as structured domain events: `ferment-v2:started`, `ferment-v2:replaced`, `ferment-v2:edited`, `ferment-v2:resumed`, `ferment-v2:completed`, `ferment-v2:blocked`, `ferment-v2:paused`, `ferment-v2:cleared`, `ferment-v2:budget_limited`, `ferment-v2:stalled`, `ferment-v2:agent_error`, and `ferment-v2:evaluated`. Replacement emits `replaced` for the outgoing ID with `replacementFermentV2Id`, then `started` for the incoming ID, so every V2 ID has one cohort start. Built-in telemetry subscribes to all external V2 events and records only bounded IDs, version, revision, lifecycle state, lifecycle reason, replacement ID, token/time counters, evaluator verdict, evaluator model, timeout/correction/request counts, HTTP status, token buckets, and cost. It also consumes the internal `ferment-v2:context_changed` event to attach active V2 ID/version/revision/status to ambient session, error, and tool telemetry while the run is active. It never emits objectives, evaluator explanations, blocked free text, prompts, filenames, or raw provider errors. Evaluator unavailability and intentional cancellation both emit `ferment-v2:evaluated`; operational failure rates must exclude `failureType=cancelled`.
