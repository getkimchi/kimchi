...
it("exits --print with code 0 when the evaluator returns no parseable verdict, instead of hanging", {
	...
	try {
		...
		// One agent turn (create_todos, then a follow-up settling reply) plus
		// one evaluator call: 2 completions for the turn + 1 for the
		// evaluator, matching the first three requests of the happy-path
		// script above before that test's evaluator returns "continue".
		expect(
