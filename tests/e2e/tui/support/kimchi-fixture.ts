...
export function launchKimchi(
	...
	terminal.submit(
		[
			...
			// Disable startup network hooks (self-update probe) so the session
			// boots without background HTTP or synchronous tar/exec work.
			// Keeps the TUI e2e hermetic and its timing deterministic.
