...
test("find a session by conversation text, inspect its dates, and continue it", async ({ terminal }) => {
	await runKimchiSession(
		...
		async (fixture, trace) => {
			...
			expect(initial).toContain("ctrl+e hide evaluators")
...
test("CLI resume keeps a search while expanding to all folders and resumes the matching project", async ({
	...
	await runKimchiSession(
		...
		async (_fixture, trace) => {
			expect(viewText(terminal)).toContain("ctrl+e hide evaluators")
			...
			expect(viewText(terminal)).toContain("ctrl+e show evaluators")
			terminal.write("\x13")
			await waitForText(terminal, "Current folder · Last active", { full: false })
			expect(viewText(terminal)).not.toContain("Internal evaluator fixture")
			terminal.write("\x05")
			await waitForText(terminal, "ctrl+e hide evaluators", { full: false })
			expect(viewText(terminal)).toContain("Current folder · Last active")
			expect(viewText(terminal)).toContain("[evaluator] Internal evaluator fixture")
			expect(viewText(terminal)).not.toContain("└─")
			terminal.write("\x1bOS")
			terminal.write("\x13")
			await waitForText(terminal, "Current folder · Threads", { full: false })
			expect(viewText(terminal)).toContain("└─ [evaluator] Internal evaluator fixture")
			trace.step("sorting and evaluator visibility are independent; F4 has no effect")
