...
test("find a session by conversation text, inspect its dates, and continue it", async ({ terminal }) => {
	await runKimchiSession(
		...
		async (fixture, trace) => {
			...
			expect(initial).toContain("└─ [background check] Internal evaluator fixture")
			expect(initial).toContain("ctrl+e hide background checks")
...
test("CLI resume keeps a search while expanding to all folders and resumes the matching project", async ({
	...
	await runKimchiSession(
		...
		async (_fixture, trace) => {
			expect(viewText(terminal)).toContain("ctrl+e hide background checks")
			const tree = viewText(terminal)
			expect(tree).toContain("Current folder · Threads")
			expect(tree).toContain("└─ [background check] Internal evaluator fixture")
			expect(tree.indexOf("Fix pool timeouts")).toBeLessThan(tree.indexOf("└─ [background check]"))
			terminal.keyDown()
			...
			expect(viewText(terminal)).toContain("Current folder · Threads")
			expect(viewText(terminal)).toContain("ctrl+e show background checks")
			expect(viewText(terminal)).not.toContain("ctrl+s")
			terminal.write("\x13")
			terminal.write("\x05")
			await waitForText(terminal, "ctrl+e hide background checks", { full: false })
			expect(viewText(terminal)).toContain("Current folder · Threads")
			expect(viewText(terminal)).toContain("└─ [background check] Internal evaluator fixture")
			terminal.write("\x1bOS")
			terminal.write("pool")
			await waitForText(terminal, "ctrl+s sort: last active", { full: false })
			terminal.write("\x13")
			await waitForText(terminal, "ctrl+s sort: best match", { full: false })
			terminal.write("\x15")
			await waitForText(terminal, "Current folder · Threads", { full: false })
			expect(viewText(terminal)).not.toContain("ctrl+s")
			expect(viewText(terminal)).toContain("└─ [background check] Internal evaluator fixture")
			trace.step("one background-check toggle; clearing search restores the parent tree")
