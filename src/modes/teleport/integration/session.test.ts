...
import { RemoteAgentSession } from "../proxy/agent-session.js"
import { ReconnectSupervisor } from "../ws/reconnect.js"
...
describe("RemoteAgentSession WebSocket integration", () => {
	it("prompt round-trip with real WS server", async () => {
		...
		const transport = await WebSocketTransport.create(wsUrl, { Authorization: "Bearer test-token" })
	...
	it("reconnect supervisor swaps client on new connection", async () => {
		...
		const transport1 = await WebSocketTransport.create(`ws://localhost:${port1}`, {
		...
		const transport2 = await WebSocketTransport.create(`ws://localhost:${port2}`, {
			Authorization: "Bearer test-token",
		})
