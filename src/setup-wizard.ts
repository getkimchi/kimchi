...
export async function runSetupWizard(options: {
	...
	if (!existsSync(getSuperpowersVendorDir())) {
		const installSuperpowers = await clack.select({
			message: "Install superpowers skills?",
			initialValue: "yes",
			options: [
				{
					value: "yes",
					label: "Yes (recommended)",
					hint: "A skill library that guides AI agents to be more effective — https://github.com/obra/superpowers",
				},
				{ value: "no", label: "No" },
			],
		})

		if (!clack.isCancel(installSuperpowers) && installSuperpowers === "yes") {
