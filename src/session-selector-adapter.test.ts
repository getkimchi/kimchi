...
describe("resume explorer using the upstream selector", () => {
	...
	it.each([
		...
		"search",
		"recent search",
	])("keeps evaluator visibility independent of %s sorting and ignores F4", async (mode) => {
		...
		const { component } = await picker([evaluator, parent])
		if (mode.includes("search")) component.handleInput("pooling")
		if (mode === "recent search") component.handleInput("\x13")
		const heading = text(component).split("\n")[1]
		const beforeF4 = text(component)
		component.handleInput("\x1bOS")
		expect(text(component)).toBe(beforeF4)
		component.handleInput("\x1b[14~")
		expect(text(component)).toBe(beforeF4)
		component.handleInput("\x05")
		expect(text(component).split("\n")[1]).toBe(heading)
		expect(text(component)).not.toContain("[background check]")
		expect(text(component)).toContain("ctrl+e show background checks")
		component.handleInput("\x13")
		const hiddenHeading = text(component).split("\n")[1]
		if (mode === "threaded") expect(hiddenHeading).toBe(heading)
		else expect(hiddenHeading).not.toBe(heading)
		expect(text(component)).not.toContain("[background check]")
		component.handleInput("\x05")
		expect(text(component).split("\n")[1]).toBe(hiddenHeading)
		expect(text(component)).toContain("[background check]")
		expect(text(component)).toContain("ctrl+e hide background checks")
	})

	it("keeps parent grouping fixed and offers sorting only while searching", async () => {
		const parent = session("parent", { modified: new Date("2026-09-01") })
		const child = session("child", { parentSessionPath: parent.path })
		const { component } = await picker([child, parent])
		expect(text(component)).not.toContain("ctrl+s")
		expect(text(component)).toContain("ctrl+e hide background checks")
		const tree = text(component)
		expect(tree).not.toContain("ctrl+p path")
		expect(tree).not.toMatch(/Last active\s+Created\s+Session/)
		expect(tree).toContain("Created:")
		component.handleInput("\x13")
		expect(text(component)).toBe(tree)
		component.handleInput("pooling")
		expect(text(component)).toContain("ctrl+s sort: last active")
		component.handleInput("\x13")
		expect(text(component)).toContain("Current folder · Last active")
		expect(text(component)).toContain("ctrl+s sort: best match")
		...
		component.handleInput("\x06")
		expect(text(component)).toContain("ctrl+f file (on)")
		expect(text(component)).toContain("File:")
		component.handleInput("\x06")
	...
	it("finds an internal session with a missing parent and resolves its parent after switching scope", async () => {
		...
		expect(text(component)).toContain("[background check] Ferment V2 evaluator")
		component.handleInput("\x05")
	...
	it("keeps internal visibility through sort, named-only filtering and cancelled dialogs", async () => {
		...
		component.handleInput("\x1b")
		component.handleInput("\x13")
		expect(text(component)).toContain("Threads · Named only")
		expect(text(component)).toContain("[background check]")
		component.handleInput("\x05")
		expect(text(component)).toContain("No named sessions")
		component.handleInput("\x0e")
		expect(text(component)).toContain("1/1 sessions · 1 background check hidden")
	})
	...
	it("shows internal children as a tree by default and restores the parent when hiding", async () => {
		...
		expect(tree).toContain("ctrl+e hide background checks")
		...
		expect(rows[1]).toContain("├─ [background check] Ferment V2 evaluator")
		expect(rows[2]).toContain("└─ [background check] Ferment V2 evaluator")
		component.handleInput("\x1b[B")
		component.handleInput("\x1b[B")
