...
describe("resume explorer using the upstream selector", () => {
	it.each([
		...
		if (mode === "flat" || mode === "recent search") component.handleInput("\x13")
		...
		expect(text(component)).not.toContain("[evaluator]")
		expect(text(component)).toContain("ctrl+e show evaluators")
		component.handleInput("\x13")
		const hiddenHeading = text(component).split("\n")[1]
		expect(hiddenHeading).not.toBe(heading)
		expect(text(component)).not.toContain("[evaluator]")
		component.handleInput("\x05")
		expect(text(component).split("\n")[1]).toBe(hiddenHeading)
		expect(text(component)).toContain("[evaluator]")
		expect(text(component)).toContain("ctrl+e hide evaluators")
	})

	it("advertises working file/evaluator controls and skips redundant empty-query sort states", async () => {
		const parent = session("parent", { modified: new Date("2026-09-01") })
		...
		expect(text(component)).toContain("ctrl+s sort: group by parent")
		component.handleInput("\x13")
	...
	it("shows internal children as a tree by default and restores the parent when hiding", async () => {
		...
		expect(tree).toContain("ctrl+e hide evaluators")
