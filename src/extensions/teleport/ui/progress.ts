...
export function createTeleportProgress(ui: ExtensionUIContext) {
	...
	function installInputLock(): void {
		unsubInput = ui.onTerminalInput((data) => {
	...
	function teardownInputLock(): void {
		unsubInput?.()
	...
	return {
		...
		// Temporarily release the input lock + hide the "🔒" widget so other UI
		// elements (e.g. the git-token prompt) can receive keystrokes. Idempotent.
		pauseInput(): void {
			if (!unsubInput) return
			teardownInputLock()
		},
		// Reinstall the input lock. No-op after finish(). Idempotent.
		resumeInput(): void {
			if (finished) return
			if (unsubInput) return
			installInputLock()
