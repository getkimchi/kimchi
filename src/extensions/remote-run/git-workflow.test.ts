...
describe("promptForRemoteBranch", () => {
	it("prompts with the suggested branch prefilled and returns the intent", async () => {
		...
		expect(ctx.ui.input).toHaveBeenCalledWith(REMOTE_BRANCH_PROMPT, "kimchi/suggested")
	...
	it("resolves the base branch from the local clone when no resolver is injected", async () => {
		...
		const result = await promptForRemoteBranch(ctx, "kimchi/suggested")
		expect(result).toEqual({ branch: "kimchi/my-branch", baseBranch: undefined })
	})
	...
	it("returns undefined on Escape (input cancelled)", async () => {
		const ctx = createContext()
	...
	it("returns undefined on empty or whitespace-only input (plain run)", async () => {
		const ctx = createContext()
	...
	it("rejects an invalid branch name with a warning and falls back to plain run", async () => {
		const ctx = createContext()
