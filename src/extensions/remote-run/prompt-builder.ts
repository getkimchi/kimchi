...
export function buildRemotePlanPrompt(planText: string, opts: RemotePlanPromptOptions): string {
	const instructions =
		opts.origin === "ferment"
			? "The user approved the following plan and wants it executed as a ferment. Start a ferment with this plan and execute it — scope it using the plan's structure (goal, constraints, chunks), activate the first phase, and work through every step. The plan text below follows the shared plan format with ## Goal, ## Constraints, and ## Chunks sections."
			: "The user approved the following plan. Execute it now — work through every chunk and verify your work."
