...
import { CUSTOM_BRANCH_ITEM, CUSTOM_BRANCH_PROMPT } from "./git-workflow.js"
import {
...
describe("buildRemotePlanPrompt", () => {
	...
	describe("buildRemotePlanPromptWithIntent", () => {
		...
		it("suggests the slug derived from the plan Goal as the first menu item", async () => {
			const ctx = createContext()
			...
			expect(ctx.ui.select).toHaveBeenCalledWith(expect.any(String), ["kimchi-build-a-feature", CUSTOM_BRANCH_ITEM])
