...
describe("goal command", () => {
	...
	it("clears only with the canonical spelling", () => {
		...
		for (const objective of ["stop", "off", "reset", "none", "cancel"]) {
			expect(parseGoalCommand(objective)).toEqual({ action: "set", objective })
		}
	})
	...
	it("formats the empty state and every goal status", () => {
		...
		for (const status of ["active", "paused", "blocked", "budget_limited", "complete"] satisfies GoalStatus[]) {
			...
			expect(summary).toContain("Usage: <1m · 1.5k tokens")
		}
	})
	...
	it("shows evaluation details only in the full command summary", () => {
		...
		expect(formatGoalSummary(evaluated)).toContain("Evaluations: 2\nLast evaluation: continue — missing smoke test")
	...
	it("formats accounting time in minutes and hours", () => {
		...
		expect(formatGoalAccounting(goal("active"))).toBe("<1m · 1.5k tokens")
		expect(formatGoalAccounting({ ...goal("active"), timeUsedMs: 19 * 60_000 })).toBe("19m · 1.5k tokens")
		expect(formatGoalAccounting({ ...goal("active"), timeUsedMs: 65 * 60_000 })).toBe("1h 5m · 1.5k tokens")
		expect(formatGoalAccounting({ ...goal("active"), tokenBudget: 2_000 })).toBe("<1m · 1.5k/2.0k tokens")
	})
})
