...
describe("goal reducer", () => {
	...
	it("round-trips privacy-safe immutable evaluator usage without changing Goal state", () => {
		const goal = createGoal(undefined, "ship", "goal-a", T1)
		const usage = { input: 10, output: 5, cacheRead: 2, cacheWrite: 1, totalTokens: 18, costUsd: 0.33 }
		const entry = putGoalEvaluatorUsageEntry("session-a", goal.id, goal.revision, usage)
		...
		expect(restoreGoal([putGoalEntry(goal), entry])).toEqual(goal)
	})
