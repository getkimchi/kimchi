...
export default function councilExtension(pi: ExtensionAPI): void {
	...
	const recordRun = (record: CouncilRunRecord): void => {
		try {
			...
			debugLog("failed to append council_run session entry", error)
		}
	}
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		const validationCatalog = buildValidationCatalog(sessionCwd)
		sessionRoutes.set(activeSessionId, {
			...
			transaction: new CouncilTransactionRuntime(sessionCwd, undefined, validationCatalog),
			progressUI,
		})
		registerCouncilTransactionTools(
			pi,
			sessionCwd,
			(toolContext) => routeForContext(toolContext)?.transaction,
			validationCatalog,
		)
	...
	pi.on("tool_execution_start", (event, ctx) => {
		...
		const checkId =
			event.args && typeof event.args === "object"
				? (event.args as { council_check_id?: unknown }).council_check_id
				: undefined
		if (
			route.transaction.state === "post_apply_checks" &&
			event.toolName === "bash" &&
			typeof checkId === "string" &&
			route.transaction.isExpectedPostApplyCheck(checkId)
		) {
			route.pendingPostApplyValidationCalls.set(event.toolCallId, checkId)
		}
	})
	pi.on("tool_execution_end", async (event, ctx) => {
		...
		const checkId = route.pendingPostApplyValidationCalls.get(event.toolCallId)
		if (checkId !== undefined) {
			route.pendingPostApplyValidationCalls.delete(event.toolCallId)
			await route.transaction.recordPostApplyCheck(event.toolName, checkId, !event.isError)
		}
	})
