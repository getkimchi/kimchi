...
describe("Council candidate redaction", () => {
	...
	it("keeps candidate evidence exact across many random content hashes", async () => {
		// Base-content hashes are effectively random 64-hex strings. The PII/secret guards can
		// false-positive on a bare hash (it is dense enough to look like crypto material), and when
		// two guards disagree about the match span the engine can consume characters past the hash —
		// including the following line break. Repeating with fresh content on every iteration exercises
		// many distinct hash byte patterns so a regression here isn't masked by one lucky hash.
		for (let i = 0; i < 25; i++) {
			const root = await temporaryRoot()
			await writeFile(join(root, "a.mjs"), `export const original = ${i}\n`)
			const transaction = new ChangeTransaction(root)
			await transaction.stageWrite("a.mjs", `export const updated = ${i}\n`)
			const candidate = transaction.changeSet()

			const compiled = await compileCouncilContext({
				context: { messages: [{ role: "user", content: "Update the value", timestamp: 1 }] },
				runId: `redaction_stress_${i}`,
				candidate,
			})
			const artifact = compiled.artifacts.find(
				(item): item is CandidatePatchArtifact => item.kind === "candidate_patch",
			)
			expect(artifact?.candidate_patch.patch).toBe(candidate.patch)
		}
	})
