...
describe("councilExtension", () => {
	...
	it("counts only the catalog-bound post-apply validation check ID as a check", async () => {
		const state = vi.spyOn(CouncilTransactionRuntime.prototype, "state", "get").mockReturnValue("post_apply_checks")
		const expectedCheck = vi
			.spyOn(CouncilTransactionRuntime.prototype, "isExpectedPostApplyCheck")
			.mockImplementation((checkId) => checkId === "package.test" || checkId === "package.typecheck")
		...
		try {
			...
			toolStart(
				{ toolName: "bash", toolCallId: "test", args: { command: "pnpm test", council_check_id: "package.test" } },
				ctx,
			)
			await toolEnd({ toolName: "bash", toolCallId: "test", isError: false }, ctx)
			expect(recordCheck).toHaveBeenLastCalledWith("bash", "package.test", true)

			toolStart(
				{
					toolName: "bash",
					toolCallId: "failed",
					args: { command: "pnpm run typecheck", council_check_id: "package.typecheck" },
				},
				ctx,
			)
			await toolEnd({ toolName: "bash", toolCallId: "failed", isError: true }, ctx)
			expect(recordCheck).toHaveBeenLastCalledWith("bash", "package.typecheck", false)
			expect(recordCheck).toHaveBeenCalledTimes(2)
		} finally {
			await sessionShutdown({}, ctx)
			state.mockRestore()
			expectedCheck.mockRestore()
	...
	it("abandons the previous transaction before replacing a session route", async () => {
		...
		try {
			await sessionStart({}, { modelRegistry: registry, sessionManager: { getSessionId: () => "route-a" } })
			await sessionStart({}, { modelRegistry: registry, sessionManager: { getSessionId: () => "route-b" } })
