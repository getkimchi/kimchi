...
import { FusionAnalysisSchema } from "./schemas.js"
...
describe("CouncilSessionCache", () => {
	...
	it("bounds entry count and byte size", () => {
		...
		expect(cache.setResult({ ...key, role: "analyst" }, { value: "b".repeat(20) }, () => true)).toBe(true)
		expect(cache.setResult({ ...key, role: "synthesis" }, { value: "c".repeat(20) }, () => true)).toBe(true)
		expect(cache.snapshot().entries).toBeLessThanOrEqual(2)
		expect(cache.snapshot().bytes).toBeLessThanOrEqual(80)
		expect(cache.setResult({ ...key, modelId: "physical/oversized" }, { value: "x".repeat(100) }, () => true)).toBe(
			false,
		)
	})
