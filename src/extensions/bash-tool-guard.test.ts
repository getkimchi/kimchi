...
describe("bashToolGuardExtension - warn sendMessage delivery", () => {
	it("delivers warn message with deliverAs: steer", async () => {
		const { default: bashToolGuardExtension } = await import("./bash-tool-guard.js")
		...
		// Fire tool_call with a bash command that triggers a warn decision
		for (const h of handlers.get("tool_call") ?? []) {
			h({ toolName: "bash", input: { command: "cat foo.ts" } })
		}
