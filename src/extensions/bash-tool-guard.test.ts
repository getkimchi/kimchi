...
describe("bashToolGuardExtension - variant suppression", () => {
	...
	function createPi() {
		...
		const tools: { name: string; description: string }[] = [{ name: "bash", description: "old bash description" }]
		const pi = {
		...
		return { pi, handlers, sendMessage, tools }
	}
	...
	it("registers no handlers when suppressBashToolGuard is true", () => {
		...
		bashToolGuardExtension(pi as never)
	...
	it("does not apply TOOL_PREFERENCES_BLOCK or bash description override when suppressBashToolGuard is true", () => {
		mockResolvePromptVariant.mockReturnValue({ name: "spicy", suppressBashToolGuard: true })
		const { pi, handlers, tools } = createPi()
		bashToolGuardExtension(pi as never)
		// Fire session_start (no-op because no handlers registered)
		for (const h of handlers.get("session_start") ?? []) h({}, {})
		// bash description must be unchanged
		expect(tools[0].description).toBe("old bash description")
	})

	it("does not steer on bash tool_call when suppressBashToolGuard is true", () => {
		mockResolvePromptVariant.mockReturnValue({ name: "spicy", suppressBashToolGuard: true })
		const { pi, handlers, sendMessage } = createPi()
		bashToolGuardExtension(pi as never)
		// Fire tool_call with a command that would normally steer (no-op because no handlers registered)
		for (const h of handlers.get("tool_call") ?? []) h({ toolName: "bash", input: { command: "cat foo.ts" } })
		expect(sendMessage).not.toHaveBeenCalled()
	})

	it("registers handlers and steers normally with the default variant", () => {
		mockResolvePromptVariant.mockReturnValue({ name: "default" })
		const { pi, handlers } = createPi()
		bashToolGuardExtension(pi as never)
		expect(handlers.get("session_start")).toBeDefined()
		expect(handlers.get("input")).toBeDefined()
