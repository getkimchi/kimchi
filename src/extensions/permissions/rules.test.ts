...
describe("remembered scope round-trip", () => {
	// The scope suggester normalizes commands (env prefix PRESERVED,
	// quotes/whitespace normalized), so the rule it stores must match the
	...
	const commands = [
		...
		"go build ./...",
		"cargo build --release",
		"GOWORK=off go test -race -timeout 30s -count=1 ./controllers/discovery/... 2>&1",
