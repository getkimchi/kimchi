...
// FOO=bar assignments, shell-tokenizes (dropping quotes), and collapses
// whitespace. Env-STRIPPING: used by extractBashProgram and the hard-block /
// read-only / bare-rule-auto-rewrite callers, where env-transparency is
// correct. The remembered-rule scope/match pair uses rememberedScopeTokens
// instead, which PRESERVES env.
...
// (empty, env-only, or backtick-poisoned). Distinct from bashCommandTokens,
// which strips env for hard-block / read-only / auto-rewrite callers where
...
// resolves (split on `| ; && ||`), with env assignments dropped and
// quotes/whitespace normalized. Used by DENY matching, which checks every
// segment so a denied program behind a pipe still blocks. (allow matching
// stays single-segment via rememberedScopeTokens — it must not widen an
// approval to a piped tail.) NOTE: this inherits `parseCommandSegments`
