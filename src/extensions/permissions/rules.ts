...
// Broad-scope rules (legacy `prefix:*` and any wildcard) must NOT fall back to
// a raw `startsWith`/regex match when the canonical form is null — otherwise
// `go test:*` (or `go *`) would match `go test | sh`, because the raw command
// starts with `go test `. So those rules require a non-null canonical up
// front. Only an exact literal rule may match the raw command directly: that
