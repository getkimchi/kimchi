...
describe("Ferment V2 evaluator", () => {
	...
	it("uses Auto's concrete model for evaluation and provenance", async () => {
		const ctx = evaluatorContext(undefined, false, model("kimchi-dev", "auto"))
		const routed = model("kimchi-dev", "routed")
		setAutoRoutingState(ctx.sessionManager.getSessionId(), { status: "resolved", model: routed })
		completeMock.mockResolvedValue(assistant('{"verdict":"continue","reason":"missing smoke test"}'))
		expect(resolveFermentV2EvaluatorModel(ctx)).toBe(routed)
		await expect(evaluateFermentV2({ objective: "ship it", messages: [], todos: [] }, ctx)).resolves.toMatchObject({
			verdict: "continue",
			model: "kimchi-dev/routed",
		})
		expect(ctx.modelRegistry.getApiKeyAndHeaders).toHaveBeenCalledWith(routed)
		expect(completeMock.mock.calls[0]?.[0]).toBe(routed)
	})

	it("does not call unresolved Auto for evaluation", async () => {
		const ctx = evaluatorContext(undefined, false, model("kimchi-dev", "auto"))
		expect(resolveFermentV2EvaluatorModel(ctx)).toBeUndefined()
		await expect(evaluateFermentV2({ objective: "ship it", messages: [], todos: [] }, ctx)).resolves.toMatchObject({
			verdict: "unavailable",
	...
	it("uses the concrete session model directly", () => {
