...
function formatPartialSnapshot(
	...
	const probeLines = [
		...
		...facts.probes.map((fact) => `- ${quote(fact.name)}: ${quote(fact.value)}`),
	]
...
function formatSystemLines(system: SystemFacts): string[] {
	...
	// In a container the kernel line describes the host, not the image, and
	// statfs on the working directory reports the enclosing filesystem's
	// capacity — label both so agents don't budget against host values.
	...
	if (system.diskFreeBytes !== undefined)
		resourceParts.push(`${formatSize(system.diskFreeBytes)} free disk${system.container ? " (host filesystem)" : ""}`)
