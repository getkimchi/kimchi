...
export function buildSystemPrompt(options: SystemPromptBuildOptions): string {
	const { pi, tools, env, contextFiles, skills, currentModelId, registry, mode, permissionMode } = options
...
- After every tool result, produce text — either the next tool call with explicit reasoning, or a final summary. Never re-issue the same tool call after a successful result. (Workers: the JSON response protocol takes precedence over a prose final summary.)
