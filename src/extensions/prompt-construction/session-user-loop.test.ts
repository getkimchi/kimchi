...
describe("resolveHasUserLoop", () => {
	const base = { args: [] as string[], stdinIsTTY: true, stdoutIsTTY: true, acpMode: false }
	...
	it("treats print mode as userless", () => {
		expect(resolveHasUserLoop({ ...base, args: ["--print"] })).toBe(false)
	})

	it("treats protocol mode as userless", () => {
		expect(resolveHasUserLoop({ ...base, args: ["--mode", "rpc"] })).toBe(false)
	})
	...
	it("treats a container-like invocation (no TTY, task arg) as userless", () => {
		expect(resolveHasUserLoop({ args: ["do the task"], stdinIsTTY: false, stdoutIsTTY: false, acpMode: false })).toBe(
			false,
		)
	})
})
