...
Use mark_todo as the default for status changes, create_todos for the initial list, add_todo to append one item, update_todos only when the plan changes significantly, clear_todos when done. Cancel superseded work with a reason in note; keep deferred or uncertain work open. Mark the current item completed and the next in_progress as you work, and pair todo updates with work tool calls when possible. At wrap-up, status-only updates are appropriate. Keep at most one item in_progress, and preserve user-created todos and existing ids when updating. Before ending a turn that declares the work complete, mark every todo whose work is done as completed or clear the list — never leave a finished item active.`
