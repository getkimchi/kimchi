...
describe("todo prompt block", () => {
	...
	it("renders guidance without a current list", () => {
		...
		expect(block).toContain("When a blocked todo waits on user input")
		expect(block).toContain("supported ask policies are now, before_final, and later")
		expect(block).toContain("Supported question types are text, confirm, single, and multi")
		expect(block).toContain("according to their ask policy")
		expect(block).toContain("Use ask_user only for Ferment-specific prompts")
