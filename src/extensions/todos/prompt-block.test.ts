...
describe("ferment-conditional todo guidance", () => {
	...
	it("renderTodoPromptBlock includes ferment guidance when ferment is active", () => {
		...
		expect(block).toContain("sub-task todo list is OPTIONAL")
	})
	...
	it("ferment step guidance permits focused steps to skip todo lists (measured run: 22 create_todos in 28 steps manufactured churn)", () => {
		setActive(makeFerment())
		const block = __test_renderTodoPromptBlock()
		expect(block).toContain("skip the list and just do the work")
		expect(block).toContain("roughly 5+ tool calls")
		expect(block).toContain("state of record")
		expect(block).toContain("one batched update_todos call")
		expect(block).toContain("never spend a whole turn only updating todos")
	})
