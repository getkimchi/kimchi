...
describe("isWriteTodosDetails", () => {
	...
	it("rejects a primitive", () => {
		expect(isWriteTodosDetails("write_todos")).toBe(false)
	})
})
...
describe("isTodoWriteToolName", () => {
	...
	it("returns true for the legacy write_todos tool name for backward compatibility", () => {
...
describe("getWriteTodosDetails", () => {
	...
	it("reads details from a legacy write_todos toolResult message entry", () => {
		const details = validDetails()
		const entry = messageEntry("m1", "write_todos", details)
		expect(getWriteTodosDetails(entry)).toEqual(details)
	})
