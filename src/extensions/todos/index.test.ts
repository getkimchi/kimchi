...
import todosExtension, { buildOpenTodosReminder, TODO_OPEN_REMINDER_TYPE } from "./index.js"
...
const INPUT_HARNESS_SESSION_ID = "input-harness-session"

type InputHandler = (event: { source: string }, ctx: ExtensionContext) => unknown

function createInputHarness() {
	const handlers = new Map<string, InputHandler[]>()
	const ctx = {
		sessionManager: { getSessionId: () => INPUT_HARNESS_SESSION_ID },
	} as unknown as ExtensionContext
	...
	return {
		...
		fireInput(source: string) {
			for (const h of handlers.get("input") ?? []) h({ source }, ctx)
		},
	}
}
...
describe("todos open-reminder input handler", () => {
	...
	it("does not send when event.source is 'extension'", () => {
		applyWriteTodos({ todos: [{ content: "task A", status: "pending" }] }, INPUT_HARNESS_SESSION_ID)
		const { pi, sendMessage, fireInput } = createInputHarness()
		todosExtension(pi)
	...
	it("does not send when all todos are completed", () => {
		applyWriteTodos({ todos: [{ content: "task A", status: "completed" }] }, INPUT_HARNESS_SESSION_ID)
		const { pi, sendMessage, fireInput } = createInputHarness()
		todosExtension(pi)
