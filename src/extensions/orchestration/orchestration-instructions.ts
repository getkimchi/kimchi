...
8. **Test strategy** — Every package/module MUST have its own test file. Testing must cover all layers, not just the domain/service layer. Specifically: if the project has handlers/controllers, they need tests (using httptest or equivalent); if it has a CLI, it needs at least a smoke test. Target a test-to-production LOC ratio of at least 1.0. Use the language's idiomatic test patterns — for Go: map-based table-driven tests (\`map[string]struct{...}\`), \`testing.T\`, subtests; for TypeScript: describe/it blocks with clear assertions; for Python: pytest parametrize. For code using concurrency primitives, the test strategy MUST include a race/thread-safety detector (e.g. \`go test -race\`, \`-fsanitize=thread\`). **Anti-flaky rule**: tests must NEVER assert specific ordering of concurrently-produced results (e.g. which goroutine finishes first, map iteration order, channel receive order). For collections with non-deterministic ordering, assert membership or sort before comparing.
