...
The formula_budget determines which model tier to use. The actual token_budget and max_turns sent to the agent have per-tier floors to account for model verbosity (light-tier models need more tokens per unit of work):

| formula_budget | Model | token_budget | max_turns |
|---|---|---|---|
| ≤ 30,000 (trivial) | **Builder Lite** | max(formula_budget, 80,000) | 20 |
| 30,001 – 100,000 | **Builder** | max(formula_budget, 50,000) | 15 |
| > 100,000 (complex) | Escalate to heaviest available | formula_budget (capped at 150,000) | 30 |

Example: a chunk with 1 file, 3 methods, 0 tests, boilerplate wiring:
base = (1 × 8,000) + (3 × 1,500) + (0 × 1,000) = 12,500; multiplier = 1.0; formula_budget = 12,500 → Builder Lite, token_budget = 80,000, max_turns = 20.

Example: a chunk with 2 files, 6 methods, 6 tests, concurrency + signal handling:
base = (2 × 8,000) + (6 × 1,500) + (6 × 1,000) = 31,000; multiplier = 2.5; formula_budget = 77,500 → Builder, token_budget = 77,500, max_turns = 15.

When in doubt, prefer the larger budget and higher turn cap — an abort followed by a follow-up agent costs more total tokens than a generous initial budget. Light-tier models are verbose and need more turns per unit of work than standard-tier models; the per-tier floors account for this. The turn cap prevents debug-loop budget exhaustion — an agent that hasn't converged in the given max_turns is unlikely to converge with more turns. If an Agent hits its budget or turn cap, spawn a follow-up with the remaining work rather than raising the budget. The follow-up prompt must list what the first agent completed and what remains.
...
function buildRoleAssignmentsSection(roles: ModelRoles, registry?: ModelRegistry): string {
	...
	lines.push(
		formatRoleModel(
			"Builder Lite",
			"simple build tasks: boilerplate, wiring, trivial CRUD",
			roles.builderLite,
			registry,
		),
	)
