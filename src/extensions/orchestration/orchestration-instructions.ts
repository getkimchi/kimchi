...
${planBullet}
- Agents run in the background by default; spawn up to 3 independent subtasks in parallel. Use \`run_in_background: false\` only when the next step depends on the result.
...
function buildBuildPhaseDirectives(ctx: PhaseDirectiveContext): string {
	...
	lines.push(
		"- DO run up to 3 independent chunks in parallel. Agents are backgrounded by default; set run_in_background: false only for hard dependencies. Run sequential chunks one at a time.",
	)
	lines.push(
