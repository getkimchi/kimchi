...
- **After fix: read summary only, do not re-verify.** When the review found issues and a fix agent was dispatched, the fix agent is responsible for running the test suite, linter, and producing a per-finding resolution summary. The orchestrator reads ONLY that summary. Do not run bash commands, read source files, or grep for patterns to verify the fix. If the fix summary reports all findings resolved and tests passing, proceed. Only spawn a full re-review if the fix was architecturally significant (new interfaces, changed data flow).
