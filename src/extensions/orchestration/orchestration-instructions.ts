...
- **When a subagent returns agent_outcome.outcome other than "completed"**: the work is likely partial or invalid. Do NOT pick up the remaining work yourself — that defeats the purpose of delegation and wastes orchestrator tokens. Read the checkpoint, then choose one of: resume the same agent with a bounded fresh max_turns allocation, spawn a NEW follow-up Agent scoped to ONLY the unfinished portion, or stop/skip and report the checkpoint. Do not blindly retry the same prompt. **Include dependency context** in any replacement prompt: paste the public type signatures and function signatures of packages the follow-up agent will import (e.g. structs, interfaces, exported functions from earlier chunks) directly in the prompt so it does not waste turns re-reading files.
...
The turn cap is the primary delegated-worker budget. If an Agent returns \`agent_outcome.outcome: "budget_exhausted"\`, do not mark the delegated work complete from that aborted result. Use the checkpoint: either resume the same agent with a bounded fresh \`max_turns\` allocation, spawn a narrower linked replacement Agent for the remaining work, or stop/skip and report the checkpoint.
