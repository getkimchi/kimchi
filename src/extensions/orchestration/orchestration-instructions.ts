...
- **When a subagent returns agent_outcome.outcome other than "completed"**: the work is likely partial or invalid. Do NOT pick up the remaining work yourself — that defeats the purpose of delegation and wastes orchestrator tokens. Inspect agent_outcome and the checkpoint before acting. Resume the same Agent only when the remaining work is a direct continuation and preserving session context is valuable; use a changed-approach resume when the same thread still matters but the prior approach stalled; spawn a NEW follow-up Agent when the remaining work has a clean narrower task boundary; run a short finalizer resume when the work appears finished but did not return completed; or stop/skip and report when blocked or unclear. Do not blindly retry the same prompt. **Include dependency context** in any replacement prompt: paste the public type signatures and function signatures of packages the follow-up agent will import (e.g. structs, interfaces, exported functions from earlier chunks) directly in the prompt so it does not waste turns re-reading files.
...
The turn cap is the primary delegated-worker budget. If an Agent returns \`agent_outcome.outcome: "budget_exhausted"\`, do not mark the delegated work complete from that aborted result. Inspect the checkpoint and choose deliberately:

| Signal | Action |
|---|---|
| Completed outcome | Use the result or complete the linked Ferment step. |
| Budget exhausted + direct continuation | Resume the same Agent with a bounded fresh \`max_turns\` allocation and a steering prompt. |
| Budget exhausted + same thread but stalled approach | Resume the same Agent once with a changed-approach steering prompt. |
| Budget exhausted + separable remaining work | Spawn a narrower linked replacement Agent for the clean task boundary. |
...
| Failed, stopped, blocked, or unclear checkpoint | Spawn a corrected replacement only if there is a clear task boundary; otherwise stop/skip and report the checkpoint. |
