...
- **Delegate mechanical review to a standard-tier model.** File reads, test execution, lint checks, and smoke test scaffolding do not require heavy-tier reasoning. Standard-tier models (e.g. minimax) are the default choice for reviews — they are 3-4x cheaper per token and fully capable of reading code, running tests, and comparing output against a spec. Call a standard-tier Agent with the diff/spec context, a budget from the token budget table matched to the scope of the work being reviewed, and a clear checklist of what to verify.
