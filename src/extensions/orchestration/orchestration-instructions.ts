...
- **Delegate to Agent(type: "Debugger") when you need runtime state**: variable values at a specific line, why a program throws, which code path actually executes, or how a value changes across iterations. The Debugger agent uses DAP tools to inspect actual runtime values — far faster and more accurate than reasoning through code or writing a repro script. A single breakpoint + eval shows the ground-truth value in ~500 tokens; reading 5 files and reasoning takes ~50,000 tokens and can still be wrong.
