...
1. **Plan phase** — Produce a Markdown spec file in the Documents directory. The spec MUST break the work into **small, independently-buildable chunks** — each chunk should produce exactly ONE file and be completable by a subagent within its output token budget. Separate implementation from tests: one chunk for \`foo.go\`, a separate chunk for \`foo_test.go\`. This prevents debug-loop budget exhaustion where an agent writes code, runs tests, sees failures, and burns turns on fix-retry cycles. Include for each chunk: the file path, method signatures / interfaces, expected behaviour, and acceptance criteria. Chunks must be ordered so each one can build on the previous (implementation before its tests). If plan is in your strengths, write it yourself; otherwise delegate to a Plan agent (heavy-tier model with plan strength).
2. **Build phase** — Delegate **one Agent call per chunk** from the plan, not one Agent for the entire build. Each agent gets the spec file path and is told which chunk to implement. Use a model with build strength, different from the planner. If chunks are independent (no data dependency), run up to 3 build agents in parallel with run_in_background. If chunks are sequential, run them one at a time, passing the previous chunk's output as context to the next.
...
- **When a subagent aborts due to token budget**: the work is likely partially done. Do NOT pick up the remaining work yourself — that defeats the purpose of delegation and wastes orchestrator tokens. Instead, spawn a NEW follow-up Agent scoped to ONLY the unfinished portion. List what the first agent completed and what remains in the follow-up prompt. Keep the same or lower budget.
