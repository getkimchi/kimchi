...
// ---------------------------------------------------------------------------
// Effective roles — collapses to current model when multi-model is off
// ---------------------------------------------------------------------------

type ProcessWithMultiModel = NodeJS.Process & {
	__kimchiMultiModelEnabled?: boolean
	__kimchiCurrentModelRef?: string
}

/**
 * Returns the role assignments that are actually in effect right now.
 * When multi-model is disabled, every role resolves to the user's currently
 * selected model so subagents stay on one model end-to-end.
 */
export function getEffectiveModelRoles(): ModelRoles {
	const p = process as ProcessWithMultiModel
	const multiModelEnabled = p.__kimchiMultiModelEnabled ?? false
	if (multiModelEnabled) return getModelRoles()

	const currentRef = p.__kimchiCurrentModelRef ?? getModelRoles().orchestrator
	return {
		orchestrator: currentRef,
		planner: currentRef,
		builder: currentRef,
		reviewer: currentRef,
		explorer: currentRef,
...
/**
 * Returns true when planning should be delegated to a separate Planner model
 * (i.e. multi-model is on and the planner role differs from the orchestrator role).
 */
export function shouldDelegatePlanning(): boolean {
	const roles = getEffectiveModelRoles()
	return roles.planner !== roles.orchestrator
}
