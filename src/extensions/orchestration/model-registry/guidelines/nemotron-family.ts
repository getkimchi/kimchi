...
export const NEMOTRON_FAMILY_RESEARCH = `- Your training data is older than the current ecosystem; default to verifying library/runtime/build-tool version assumptions with \`web_search\`/\`web_fetch\` rather than relying on memory.
...
export const NEMOTRON_FAMILY_BUILD = "- Your long context window is a strength — read each file in full before editing."
...
export const NEMOTRON_3_ULTRA_EXPLORE = `- Your 1M token context window is your main advantage — read files in full rather than skimming.
...
export const NEMOTRON_3_ULTRA_RESEARCH = `- Your 1M token context window lets you ingest entire documentation pages or long web resources in a single pass. Prefer \`web_fetch\` for long pages rather than skimming search snippets.
...
export const NEMOTRON_3_ULTRA_BUILD = `- Stay strictly within the spec. Do NOT design, refactor, or expand scope. If the spec is ambiguous, stop and report — do not improvise.
