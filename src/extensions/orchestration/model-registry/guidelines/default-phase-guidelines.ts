...
- If you encounter an unfamiliar library, tool, file format, or config schema, run ONE targeted \`web_search\` (or switch to \`research\` phase) before forming a hypothesis. A single confirmed fact prevents building plans on bad assumptions.
...
- If the task names a specific library, framework, build tool, or vendor kit, assume your knowledge may be stale and verify the specific facts (version, API, install step, protocol) you plan to rely on with a quick \`web_search\`. Do not trust memory just because the name feels familiar.
