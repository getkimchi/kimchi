...
describe("getEffectiveModelRoles", () => {
	...
	it("returns configured roles when multi-model is on", () => {
		;(process as P).__kimchiMultiModelEnabled = true
		const roles = getEffectiveModelRoles()
		expect(roles).toEqual(DEFAULT_MODEL_ROLES)
	})

	it("collapses all roles to current model ref when multi-model is off", () => {
		;(process as P).__kimchiMultiModelEnabled = false
		;(process as P).__kimchiCurrentModelRef = "anthropic/claude-sonnet-4-5"
		const roles = getEffectiveModelRoles()
		const ref = "anthropic/claude-sonnet-4-5"
		expect(roles.orchestrator).toBe(ref)
		expect(roles.planner).toBe(ref)
		expect(roles.builder).toBe(ref)
		expect(roles.reviewer).toBe(ref)
		expect(roles.explorer).toBe(ref)
	})

	it("falls back to configured orchestrator ref when current model ref is unset and multi-model is off", () => {
		;(process as P).__kimchiMultiModelEnabled = false
		;(process as P).__kimchiCurrentModelRef = undefined
		const roles = getEffectiveModelRoles()
		expect(roles.orchestrator).toBe(DEFAULT_MODEL_ROLES.orchestrator)
		expect(roles.planner).toBe(DEFAULT_MODEL_ROLES.orchestrator)
	})
})
...
describe("shouldDelegatePlanning", () => {
	...
	it("returns false when multi-model is off (all roles collapse to same model)", () => {
		;(process as P).__kimchiMultiModelEnabled = false
		;(process as P).__kimchiCurrentModelRef = "anthropic/claude-sonnet-4-5"
		expect(shouldDelegatePlanning()).toBe(false)
	})

	it("returns false when multi-model is on but planner equals orchestrator", () => {
		;(process as P).__kimchiMultiModelEnabled = true
