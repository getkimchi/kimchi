...
import { type LastGradedPhaseInfo, deriveFermentState } from "./derive-state.js"
...
export function buildPlannerSupplement(runtime: FermentRuntime = defaultFermentRuntime): string {
	...
	if (!f) return ""
	const derived = deriveFermentState(f, runtime)
	...
	// Derives everything from the rich state struct so there's one read path.
	const selfImprovementSection = derived.lastGradedPhase ? renderSelfImprovementForPhase(derived.lastGradedPhase) : ""
...
/**
 * Render self-improvement feedback for the previous phase. Takes the derived
 * `LastGradedPhaseInfo` (already includes the grade + any corrective text the
 * runtime recorded) and feeds it through the existing feedback pipeline. The
 * corrective text may be undefined when the previous grade was not D/F —
 * `renderSelfImprovementSection` handles that case.
 */
function renderSelfImprovementForPhase(lastGraded: LastGradedPhaseInfo): string {
	const feedback = evaluatePhaseFeedback(lastGraded.grade)
	return renderSelfImprovementSection(lastGraded.grade, feedback, lastGraded.correctiveText)
}
