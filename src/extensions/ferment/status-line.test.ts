...
import { canToggleFermentStopPolicy, formatFermentStatusLineDisplay, getFermentStopLabel } from "./status-line.js"
...
describe("ferment status line", () => {
	it("hides absent and terminal ferments", () => {
		expect(formatFermentStatusLineDisplay(undefined, "manual", plainStyle)).toBeNull()
		expect(formatFermentStatusLineDisplay(makeFerment("complete"), "manual", plainStyle)).toBeNull()
		expect(formatFermentStatusLineDisplay(makeFerment("abandoned"), "manual", plainStyle)).toBeNull()
	})

	it("shows drafts without stop-policy controls", () => {
		const display = formatFermentStatusLineDisplay(makeFerment("draft"), "manual", plainStyle)
	...
	it("labels manual policy as stopping at a phase boundary", () => {
		const display = formatFermentStatusLineDisplay(makeFerment("running"), "manual", plainStyle)
	...
	it("labels automated policy as stopping at completion", () => {
		const display = formatFermentStatusLineDisplay(makeFerment("paused"), "automated", plainStyle)
