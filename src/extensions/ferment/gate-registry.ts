...
export const GATE_REGISTRY = {
	...
	P1: {
		...
		guidance: [
			...
			"",
			"For implementation steps, prefer steps with must_haves (truths/artifacts/key_links) declared —",
			"these give the S2 gate concrete targets. A step with only a description and no must_haves",
			"is acceptable for trivial work but should be flagged for non-trivial implementation.",
		].join("\n"),
	},
	P2: {
		id: "P2",
	...
	S2: {
		...
		guidance: [
			...
			"",
			"If the step has must_haves, check whether the verify command actually exercised them:",
			"  - Did it check the Truths (behavioral assertions)?",
			"  - Did it check the Artifacts (file existence + exports)?",
			"  - Did it check the Key Links (import wiring)?",
			"  A verify command that runs but doesn't exercise the must_haves is 'proxy', not 'test'.",
			"",
	...
	C3: {
		...
		guidance: [
			...
			"If steps had must_haves declared, check that at least the load-bearing ones were exercised",
			"by a smoke or test verification — not just syntactically checked.",
		].join("\n"),
	},
