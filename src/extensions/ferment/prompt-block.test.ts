...
describe("buildFermentPromptBlock", () => {
	...
	describe("rule-survival — load-bearing substrings", () => {
		...
		it("includes Upfront Contract directives mentioning propose_ferment_scoping", () => {
			...
			expect(out).toContain("If you emit non-empty `questions`, keep the accompanying phases answer-agnostic")
			expect(out).toContain("finalize those after the answers come back")
			...
			expect(out).toContain("radio/checkbox questions must have 2-5 options")
		...
		it("guides plan-shaping discovery before proposing scoping", () => {
			const out = buildFermentPromptBlock(PI_ONESHOT, makeRuntime()) ?? ""
			expect(out).toContain(
				"Default for broad existing-codebase improvement/audit/planning requests, even when the user asks with a simple prompt",
			)
			expect(out).toContain("delegate 1-4 independent Explore probes")
			expect(out).toContain("One Explore probe is valid when there is only one broad unknown")
			expect(out).toContain("use more only for genuinely independent areas")
			expect(out).toContain("Use direct reads for narrow facts")
			expect(out).toContain("short entrypoint snippets")
			expect(out).toContain("do not read large source files end-to-end before the delegation checkpoint")
			expect(out).toContain("Use Explore for broader areas that could change the plan")
			expect(out).toContain("Before calling `propose_ferment_scoping`")
			...
			expect(out).toContain("start with `token_budget: 120000`")
			...
			expect(out).toContain(
				'several concurrent, bounded Explore subagents over one broad "understand the whole project" scan',
			)
			expect(out).toContain("file map/entry points")
			expect(out).toContain("popup/UI flow")
			expect(out).toContain("background/storage/API flow")
			expect(out).toContain("risks/refactor opportunities")
