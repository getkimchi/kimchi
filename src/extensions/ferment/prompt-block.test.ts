...
describe("buildFermentPromptBlock", () => {
	...
	describe("rule-survival — load-bearing substrings", () => {
		...
		it("guides plan-shaping discovery before proposing scoping", () => {
			...
			expect(out).toContain("Before reading an implementation file end-to-end")
			expect(out).toContain("inspecting size/length or searching for the specific symbols")
			...
			expect(out).toContain("If you accidentally read an entire implementation file")
