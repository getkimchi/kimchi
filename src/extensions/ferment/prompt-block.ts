...
function buildPlannerSupplement(f: Ferment, continuationPolicy: ContinuationPolicy): string {
	...
	const delegationCheckpoint =
		"Default for broad existing-codebase improvement/audit/planning requests, even when the user asks with a simple prompt: after a small direct scan to identify the project shape, delegate 1-4 independent Explore probes before you synthesize recommendations. One Explore probe is valid when there is only one broad unknown; use more only for genuinely independent areas. Use direct reads for narrow facts and short entrypoint snippets; do not read large source files end-to-end before the delegation checkpoint. Use Explore for broader areas that could change the plan."
...
<phase_0_inventory required="true" before="any filesystem read, list, grep, bash, or codebase discovery">
First response action: print a concise list of all available skills and subagent types so the user can see what delegation surface exists. Use the visible skill list or a skill-listing tool if one is available; if skills are not exposed in this environment, say that explicitly instead of inventing names. For subagents, inspect the Agent tool \`subagent_type\` options and the available-subagent prompt section. Do not start direct reads, Explore agents, or scoping synthesis until this inventory is printed.
</phase_0_inventory>
...
Before calling \`propose_ferment_scoping\`, resolve codebase facts that materially determine the plan shape. Small, targeted direct reads are allowed when they are the fastest way to identify the project shape: file listing plus concise manifest/README/package/config reads and, if needed, short entrypoint snippets. Do not read large source files end-to-end before the delegation checkpoint. For broad improvement/audit/planning requests over an existing codebase, you MUST run a delegation checkpoint after the initial targeted reads and before drafting the plan: immediately spawn 1-4 narrow Explore subagents for independent areas that could change the recommendations, wait for their results, then synthesize. One Explore subagent is valid when there is only one broad unknown; use multiple only when there are genuinely independent areas. Good areas include popup/UI flow, background/storage/API flow, nearby related projects, risks/security/refactor opportunities, or repo-specific architecture patterns. The next action after file listing plus concise README/manifest/package/config reads is Agent, not more Read calls. Do not skip this checkpoint just because the direct scan feels sufficient. Forbidden pattern: reading large files end-to-end before the delegation checkpoint, then claiming direct analysis was sufficient. If you accidentally read a large implementation file before the checkpoint, stop direct reads immediately and spawn the required Explore subagent(s) before any more analysis or scoping. Skip only when the task is simple/greenfield or the user explicitly asks you not to delegate; record that reason in \`assumptions\`.

Use Explore subagents for broader or parallel discovery, especially work that would otherwise become an "explore", "find the existing pattern", "understand the registry", or similar discovery-only phase. Each scoping discovery Agent call should use \`subagent_type: "Explore"\` and start with \`token_budget: ${SCOPING_EXPLORE_TOKEN_BUDGET}\`; increase only if the user explicitly asks or the missing fact is genuinely plan-blocking. Keep each Explore prompt narrowly scoped to one independent area or question. Prefer several concurrent, bounded Explore subagents over one broad "understand the whole project" scan: for example, one for file map/entry points, one for popup/UI flow, one for background/storage/API flow, and one for risks/refactor opportunities. If there are multiple independent unknowns, spawn those Explore subagents concurrently with \`run_in_background: true\`. If an Explore subagent aborts on token budget, do not retry the same broad task; use any partial result, spawn a narrower replacement only if that missing fact is plan-blocking, otherwise continue with direct targeted reads or record the uncertainty in \`assumptions\`. Do not make discovery-only work a user-approved phase when that discovery is needed to decide what the approved phases should be. The plan you propose should reflect the discovered files, patterns, constraints, and implementation layer.
