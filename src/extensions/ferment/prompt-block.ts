...
On the first scoping turn after \`/ferment\`, draft \`title\`/\`goal\`/\`success_criteria\`/\`constraints\`/\`assumptions\`/\`phases\` from the user's free-form intent. The title is required and should be a concise 3-5 word Ferment name. Do not call \`propose_ferment_scoping\` yet. First spawn a subagent with \`subagent_type: "Plan Reviewer"\` and send it that exact draft.
	...
	Before every \`propose_ferment_scoping\` call, run a separate Plan Reviewer review of the exact plan. The Plan Reviewer reviews architecture fit, complexity, dependencies, risks, and verification; it does NOT implement code. Include its verdict in \`architect_review\`. You may not call \`propose_ferment_scoping\` without \`architect_review\`. If the Plan Reviewer says \`needs_revision\`, revise the plan, run Plan Reviewer again, and do NOT ask the user yet unless Plan Reviewer explicitly says a human decision is blocking. If Plan Reviewer asks a blocking clarification question, put that question in \`propose_ferment_scoping.questions\` and keep the plan provisional. If the user later gives plan feedback, treat the plan as changed and run Plan Reviewer again before presenting it again.

	The Plan Reviewer subagent prompt MUST include the full exact plan payload inside XML tags:
	<ferment_plan>
	JSON payload you intend to pass to propose_ferment_scoping, excluding architect_review
	</ferment_plan>
	Tell Plan Reviewer to return only JSON with \`status\`, \`summary\`, \`required_changes\`, optional \`reservations\`, and optional \`questions\`. If Plan Reviewer has any required changes, its status must be \`needs_revision\`; \`approved\` means \`required_changes: []\`. Plan Reviewer may use a Researcher subagent when current external docs, APIs, standards, or browser behavior materially affect the review. Do not ask Plan Reviewer to implement. Do not send Plan Reviewer a prose summary instead of the XML plan.

	\`architect_review\` must include \`status\`, \`summary\`, \`required_changes\`, optional \`reservations\`, and may include \`reviewed_plan_hash\`. Prefer \`reviewed_plan_hash: "current"\` when Plan Reviewer reviewed the exact XML plan. Do not guess or chase \`Current plan_hash\` values. If \`propose_ferment_scoping\` rejects a stale explicit hash, set \`reviewed_plan_hash\` to \`"current"\` only if the plan payload is unchanged; if the plan changed, rerun Plan Reviewer first. Never summarize the plan in chat as a substitute for the tool call.
