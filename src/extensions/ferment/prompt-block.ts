...
- Phases are set by the active subagent persona or by internal mechanisms — not by a tool the parent calls directly. If you spawn a subagent, that agent's persona wires the phase for its own run.
- Valid phases: explore, research, plan, build, review
- Phase tags are metadata-only; they are decoupled from ferment state transitions (activate_ferment_phase) and do not need to line up with them
