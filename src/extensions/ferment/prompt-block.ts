...
Do not use \`questions\` for plan approval, plan feedback, or "should I proceed/start/continue with this plan?" confirmation. That confirmation is host-owned. When no genuine decision-blocking scoping question remains, call \`propose_ferment_scoping\` with \`questions: []\`; the host renders the plan and collects start/feedback input.

Do not use \`type: "confirm"\` in \`propose_ferment_scoping.questions\`. For real yes/no scoping decisions, use \`type: "single"\` with explicit option labels; those labels can be in the user's language.
...
After \`propose_ferment_scoping\` returns "Plan ready for review", the host will collect the user's review after your turn ends. Stop there: do not call more tools or skills, do not call \`propose_ferment_scoping\` again, do not summarize the plan in chat, and do not tell the user to wait for the TUI.
