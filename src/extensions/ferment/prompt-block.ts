...
Plans are rendered by the host in markdown style with colored headings, code blocks, and bullet points. Structure plan content with markdown that takes full advantage of this rendering:
- Use ## Headings for each section (e.g. \`## Goal\`, \`## Success Criteria\`, \`## Constraints\`, \`## Assumptions\`, \`## Phases\`)
- Use **bold** for phase names (e.g. \`**Phase 1: Implementation**\`)
- Use - bullet lists for constraints, assumptions, and success criteria items
- Use \`code\` for file paths, tool names, and command snippets
- Keep step descriptions as plain text (they render in a separate step list, not in the plan markdown itself)
