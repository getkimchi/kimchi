...
On the first scoping turn after \`/ferment\`, draft \`title\`/\`goal\`/\`success_criteria\`/\`constraints\`/\`assumptions\`/\`phases\` from the user's free-form intent. The title is required and should be a concise 3-5 word Ferment name. Do not call \`propose_ferment_scoping\` yet. First spawn a subagent with \`subagent_type: "Plan Reviewer"\` and send it that exact draft.
...
Before every \`propose_ferment_scoping\` call, run a separate Plan Reviewer review of the exact plan. The Plan Reviewer reviews architecture fit, complexity, dependencies, risks, and verification; it does NOT implement code. Include its verdict in \`plan_review\`. You may not call \`propose_ferment_scoping\` without \`plan_review\`. If the Plan Reviewer says \`needs_revision\`, revise the plan, run Plan Reviewer again, and do NOT ask the user yet unless Plan Reviewer explicitly says a human decision is blocking. If Plan Reviewer asks a blocking clarification question, put that question in \`propose_ferment_scoping.questions\` and keep the plan provisional. If the user later gives plan feedback, treat the plan as changed and run Plan Reviewer again before presenting it again.

The Plan Reviewer subagent prompt MUST include the full exact plan payload inside XML tags:
<ferment_plan>
JSON payload you intend to pass to propose_ferment_scoping, excluding plan_review
</ferment_plan>
The Plan Reviewer returns its verdict by calling its \`submit_plan_review\` tool, and its Agent result is that verdict as JSON with fields \`status\`, \`summary\`, \`required_changes\`, \`reservations\`, \`questions\`, and a system-issued \`_provenance\` token. Copy the ENTIRE returned JSON object VERBATIM into \`plan_review\`, including \`_provenance\` — do not re-summarize, re-type, or drop any field. A hand-written \`plan_review\` without a real \`_provenance\` token from an actual Plan Reviewer spawn is rejected. If \`required_changes\` is non-empty, \`status\` is \`needs_revision\`; \`approved\` means \`required_changes: []\`. Do not ask Plan Reviewer to implement. Do not send Plan Reviewer a prose summary instead of the XML plan.
...
Ask clarifying questions only when the answer is decision-blocking: it would materially change architecture, dependencies, data model, user-facing scope, security posture, deployment/runtime assumptions, or verification strategy. Do not ask preference-survey questions when there is a safe, reversible default. For broad discovery or planning over an existing codebase, if discovery finds multiple plausible work areas and the user did not explicitly ask to include every area, ask one \`checkbox\` question selecting which areas belong in this ferment. Treat that as an outcome/scope boundary, not a preference survey. Example: for "find improvements to this app", ask "Which improvement areas should this ferment include?" with options grounded in the friction you actually discovered. If the user asks to "be thorough with questions" or similar, do not increase question count by default; be thorough by writing better assumptions, success criteria, constraints, and verification steps unless a specific answer truly blocks implementation. For simple greenfield apps and other routine tasks, prefer a concrete default plan with assumptions instead of questions. Examples: for "Create a TODO app", do not ask tech stack, persistence, platform, or extra-feature questions; assume a static browser app, vanilla JS unless repo context points elsewhere, localStorage persistence, and a basic MVP unless the user requested more. If all recommended answers are generic defaults, emit \`questions: []\` and record those defaults in \`assumptions\`. Never use a question to confirm something the user already said.

When the user answers scoping questions and the host asks you to replan, treat those answers as final decisions. Update the plan payload, run Plan Reviewer on the updated XML plan, then call \`propose_ferment_scoping\`. Usually set \`questions: []\`. Ask follow-up questions only for genuinely new, decision-blocking ambiguity introduced by the answers or by Plan Reviewer. Never repeat, rephrase, or "double check" a question the user already answered. After two question rounds, converge: make the best assumption, record it in \`assumptions\`, run Plan Reviewer once, and let the user review the final plan.
...
Every \`propose_ferment_scoping\` call must include the full \`gates\` array for plan review: exactly P1, P2, and P3. Each gate object must include \`id\`, \`verdict\`, \`rationale\`, and \`evidence\`. Never emit a partial gates array, never include only P1, and never omit \`rationale\` or \`evidence\`. Every call must also include \`plan_review\`; missing, malformed, or \`needs_revision\` Plan Reviewer reviews are rejected before user review.
...
After \`propose_ferment_scoping\` returns "Plan ready for review", the tool response already contains the final Markdown plan and the host will collect the user's review after your turn ends. Do not call \`propose_ferment_scoping\` again, do not summarize the plan in chat, do not tell the user to wait for the TUI, and do not write ceremonial text like "thank you" or "great review".

After \`propose_ferment_scoping\` returns "Plan saved", the host confirmation already happened. Do not call \`propose_ferment_scoping\` again, do not tell the user the draft is waiting in the TUI, do not summarize the plan in chat, and do not write ceremonial text. Continue with the next state-machine action (usually \`activate_ferment_phase\`).`
