...
describe("runScopingFlow", () => {
	it("single non-empty input → markScopingInteractive called + sendMessage fired with intent embedded in content", async () => {
		...
		expect(text).toContain("Spawn 1-4 narrow Explore subagents")
		...
		expect(text).toContain("Explore subagent contract:")
		...
		expect(text).toContain("not in chat")
		expect(text).toContain("Do not call propose_ferment_scoping first and then ask follow-up questions in prose")
		...
		expect(text).toContain("id, verdict, rationale, and evidence")
		expect(text).toContain("Never emit a partial gates array")
