...
export function formatNextActionHint(ferment: Ferment): string | undefined {
	...
	switch (action.kind) {
		...
		case "complete_step": {
			const label = stepLabel ?? `step "${action.stepId}"`
			return `Next action: call \`${toolName}\` for ${label} with the summary and gates \u2014 ferment_id "${ferment.id}", phase_id "${action.phaseId}", step_id "${action.stepId}"${verifyHint}. For direct execution, omit worker_agent_id. If you delegated, include worker_agent_id only after the linked worker has a completed outcome and completed report.`
		}
		case "verify_step": {
			const label = stepLabel ?? `step "${action.stepId}"`
