...
export function createAutoModelExtension(options: AutoModelExtensionOptions = {}): ExtensionFactory {
	return (pi: ExtensionAPI) => {
		...
		pi.on("session_start", async (event, ctx) => {
			...
			const freshSession =
				event.reason === "new" || (event.reason === "startup" && !entries.some((entry) => entry.type === "message"))
			const explicitLaunchChoice =
				event.reason === "startup" &&
				(cliOptions?.model || cliOptions?.provider || cliOptions?.["multi-model"] || ctx.scopedModels.length > 0)
