...
// Bundled SKILL.md content — inlined so it works in both dev and Bun binary.
// Update this string whenever SKILL.md changes.
const SKILL_MD = `---
name: improve
...
const SKILL_PATH = join(homedir(), ".config", "kimchi", "harness", "skills", "improve", "SKILL.md")

export default function improveExtension(_pi: ExtensionAPI): void {
	// Deploy SKILL.md to harness on startup, write-only-if-different.
	try {
		const current = readFileSync(SKILL_PATH, "utf-8")
		if (current !== SKILL_MD) writeFileSync(SKILL_PATH, SKILL_MD)
	} catch {
