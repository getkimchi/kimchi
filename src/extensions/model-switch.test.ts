...
describe("post-compaction model switch", () => {
	...
	it("allows switch to smaller context model after compaction refreshes latestMessages", async () => {
		// Simulate a session with a large pre-compaction context
		const largeMessages = Array.from({ length: 50 }, () => ({
			role: "user" as const,
			content: [{ type: "text" as const, text: "x".repeat(10_000) }],
			timestamp: 0 as const,
		}))
		// 50 * ceil(10000/4) = 50 * 2500 = 125_000 tokens estimated
		__setLatestMessagesForTest(largeMessages)

		const { pi, trigger, setModel } = createHarnessWithTrigger()
		modelSwitchExtension(pi)

		// Before compaction: switch to minimax (100k window) should be rejected
		// because estimated tokens (125k) > safe window (95k)
		await trigger(
			"model_select",
			{
				type: "model_select",
		...
		expect(setModel).toHaveBeenCalledTimes(1) // reverted back

		// Simulate compaction: post-compaction messages are small
		const smallMessages = [
			{
				role: "user" as const,
		...
		__setLatestMessagesForTest(smallMessages)

		// After compaction: switch to minimax (100k window) should succeed
		// because estimated tokens (~5) < safe window (95k)
		setModel.mockClear()
		await trigger(
			"model_select",
			{
		...
		expect(setModel).not.toHaveBeenCalled() // not reverted — switch accepted
	})

	it("allows switch to non-vision model after compaction removes images", async () => {
		// Set up: session has images pre-compaction
		const { pi: guardPi, trigger: guardTrigger } = createHarnessWithTrigger()
		createModelGuardExtension(guardPi)
		const guardCtx = makeMockCtx()
		await guardTrigger(
			"context",
			{
				messages: [
		...
		// Try switching to non-vision model — should be rejected
		const { pi: switchPi, trigger: switchTrigger, setModel } = createHarnessWithTrigger()
		modelSwitchExtension(switchPi)
		await switchTrigger(
			"model_select",
			{
				type: "model_select",
		...
		expect(setModel).toHaveBeenCalledTimes(1) // reverted — blocked by vision guard

		// Simulate post-compaction state: context event fires with text-only messages
		// (In production, the session_compact handler refreshes imagesDetected by
		// scanning post-compaction messages. Here we simulate the effect by firing
		// a context event with text-only messages, which is what the next LLM call
		// would see after compaction.)
		__resetImagesDetectedForTest()
		await guardTrigger(
			"context",
			{ messages: [{ role: "user" as const, content: [{ type: "text" as const, text: "Summary." }], timestamp: 0 }] },
			guardCtx as never,
		)
		expect(sessionHasImages()).toBe(false)

		// Now switch to non-vision model should succeed
		setModel.mockClear()
		await switchTrigger(
			"model_select",
			{
