...
export function renderAgentWorkerBudgetTable(): string {
	const rows: Array<[string, AgentWorkerBudget]> = [
		["Default (single task, code or exploration)", AGENT_WORKER_BUDGETS.default],
		["Multi-file or complex implementation", AGENT_WORKER_BUDGETS.multiFile],
		["Review (read code + write findings)", AGENT_WORKER_BUDGETS.review],
