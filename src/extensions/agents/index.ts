...
function getStatusInstruction(status: string, abortReason?: AgentAbortReason): string {
	if (status === "aborted" && abortReason === "token_budget") {
		return "\nThe agent ran out of its token budget. Do not blindly retry the same prompt. Choose one: resume this agent with a bounded additional budget, spawn a narrower replacement Agent for the remaining work, or stop and report the checkpoint."
	}
	...
	if (status === "aborted" && abortReason === "max_duration") {
		return "\nThe agent exceeded its maximum allowed wall-clock duration and was terminated. Review any partial results. If the work is incomplete, resume with a bounded turn allocation or spawn a follow-up Agent scoped only to the remaining work."
	}
	if (status === "aborted" && abortReason === "max_turns") {
		return "\nThe agent exhausted its turn budget. Do not mark delegated work complete from an aborted result. Resume it with a bounded additional max_turns allocation, spawn a narrower linked replacement Agent, or stop and report the checkpoint."
	}
	return ""
}
...
- Treat budgets as hard caller constraints. If an agent aborts because of max_turns or token_budget, do not blindly retry the same prompt. Choose one of: bounded resume with a fresh max_turns allocation, narrower replacement worker, or stop/skip with the checkpoint.
