/**
 * kimchi agents extension — Claude Code-style autonomous sub-agents.
 *
 * Tools:
	...
	;(globalThis as unknown as Record<symbol, unknown>)[MANAGER_KEY] = {
		...
		spawn: (piRef: unknown, ctx: unknown, type: string, prompt: string, options: unknown) =>
			manager.spawn(
				...
				type,
				prompt,
				options as Parameters<typeof manager.spawn>[4],
			),
...
- Use model to specify a different model (as "provider/modelId", or fuzzy e.g. "kimi", "minimax").
...
- Use inherit_context if the agent needs the parent conversation history.`,
			parameters: Type.Object({
				...
				model: Type.Optional(
					Type.String({
						description:
							'Optional model override. Accepts "provider/modelId" or fuzzy name (e.g. "kimi", "minimax"). Omit to use the agent type\'s default.',
					}),
				),
	...
	function getModelLabel(type: string, registry?: ModelRegistry): string {
		...
		if (registry) {
			const resolvedM = resolveModel(cfg.model, registry)
...
model: <optional model as "provider/modelId", e.g. "kimchi-dev/minimax-m2.7". Omit to inherit parent model>
...
Write the file using the write tool. Only write the file, nothing else.`
		...
		const record = await manager.spawnAndWait(pi, ctx as ExtensionContext, "general-purpose", generatePrompt, {
	...
	async function showManualWizard(ctx: ExtensionCommandContext, targetDir: string) {
		...
		let modelLine = ""
		if (modelChoice === "kimi-k2.6") modelLine = "\nmodel: kimchi-dev/kimi-k2.6"
		else if (modelChoice === "minimax-m2.7") modelLine = "\nmodel: kimchi-dev/minimax-m2.7"
		else if (modelChoice === "nemotron-3-super-fp4") modelLine = "\nmodel: kimchi-dev/nemotron-3-super-fp4"
