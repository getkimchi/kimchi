...
describe("resolveStreamIdleTimeoutMs", () => {
	it("returns the default (120000) when the env var is unset", () => {
	...
	it("returns the default for empty, non-numeric, or negative values", () => {
		expect(resolveStreamIdleTimeoutMs({ [STREAM_IDLE_TIMEOUT_MS_ENV]: "" })).toBe(DEFAULT_STREAM_IDLE_TIMEOUT_MS)
		expect(resolveStreamIdleTimeoutMs({ [STREAM_IDLE_TIMEOUT_MS_ENV]: "banana" })).toBe(DEFAULT_STREAM_IDLE_TIMEOUT_MS)
		expect(resolveStreamIdleTimeoutMs({ [STREAM_IDLE_TIMEOUT_MS_ENV]: "-1" })).toBe(DEFAULT_STREAM_IDLE_TIMEOUT_MS)
	})

	it("returns the parsed positive value when set", () => {
		expect(resolveStreamIdleTimeoutMs({ [STREAM_IDLE_TIMEOUT_MS_ENV]: "120000" })).toBe(120000)
		expect(resolveStreamIdleTimeoutMs({ [STREAM_IDLE_TIMEOUT_MS_ENV]: "60000" })).toBe(60000)
	})

	it("exposes the documented default constant", () => {
		expect(DEFAULT_STREAM_IDLE_TIMEOUT_MS).toBe(120000)
	})
})
