...
describe("hermes tool registration", () => {
	it("registers itself with install metadata for the wizard", () => {
		...
		expect(tool?.binaryName).toBe("hermes")
		expect(tool?.configPath).toBe("~/.hermes/config.yaml")
	})
	...
	it("write() rejects when no models are available", async () => {
		const tool = byId("hermes")
		await expect(tool?.write("global", "test-key", [])).rejects.toThrow(/models/)
	})
})
...
describe("writeHermesDirect integration", () => {
	...
	it("preserves existing user config and merges provider block into YAML", async () => {
		...
		writeFileSync(join(configDir, "config.yaml"), YAML.stringify(existing), "utf-8")

		const tool = byId("hermes")
		await tool?.write("global", "test-key-123", TEST_MODELS)

		const written = YAML.parse(readFileSync(join(configDir, "config.yaml"), "utf-8")) as Record<string, unknown>

		const agents = written.agents as Record<string, unknown>
		const defaults = agents.defaults as Record<string, unknown>
		const modelMap = defaults.model as Record<string, unknown>

		expect(modelMap.temperature).toBe(0.7)
		expect(modelMap.fallbacks).toContain("other/model")
		expect(modelMap.fallbacks).toContain("kimchi/nemotron-3-super-fp4")
		expect(modelMap.fallbacks).toContain("kimchi/minimax-m2.7")

		const modelsCatalog = defaults.models as Record<string, unknown>
		expect(modelsCatalog["other/model"]).toEqual({ alias: "Other" })
		expect(modelsCatalog["kimchi/kimi-k2.6"]).toBeDefined()

		const providers = written.providers as Record<string, unknown>
		expect(providers.kimchi).toBeDefined()
		expect((providers.kimchi as Record<string, unknown>).baseUrl).toBe("https://llm.kimchi.dev/openai/v1")
	})

	it("creates config.yaml from scratch when none exists", async () => {
		mkdirSync(join(tmp, ".hermes"), { recursive: true })
		const tool = byId("hermes")
		await tool?.write("global", "test-key-123", TEST_MODELS)
		const written = YAML.parse(readFileSync(join(tmp, ".hermes", "config.yaml"), "utf-8")) as Record<string, unknown>
		expect(written.providers).toBeDefined()
		expect(written.agents).toBeDefined()
	})

	it("writes the API key to .env alongside config.yaml", async () => {
		mkdirSync(join(tmp, ".hermes"), { recursive: true })
		const tool = byId("hermes")
		await tool?.write("global", "test-key-abc", TEST_MODELS)
		expect(readFileSync(join(tmp, ".hermes", ".env"), "utf-8")).toBe("KIMCHI_API_KEY=test-key-abc\n")
	})
})
...
describe("writeHermesViaCLI integration", () => {
	...
	it("merges fallbacks via CLI without losing existing entries", async () => {
		const tool = byId("hermes")
		await tool?.write("global", "test-key-123", TEST_MODELS)
	...
	it("merges models catalog via CLI without losing existing entries", async () => {
		const tool = byId("hermes")
		await tool?.write("global", "test-key-123", TEST_MODELS)
	...
	it("writes the API key to .env when using CLI path", async () => {
		const tool = byId("hermes")
		await tool?.write("global", "cli-key-456", TEST_MODELS)
		expect(readFileSync(join(tmp, ".hermes", ".env"), "utf-8")).toBe("KIMCHI_API_KEY=cli-key-456\n")
	})
})
...
describe("detectHermes", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "kimchi-hermes-detect-"))
		prevHome = process.env.HOME
		process.env.HOME = tmp
	})
	...
	it("returns true when ~/.hermes directory exists", () => {
		mkdirSync(join(tmp, ".hermes"), { recursive: true })
		vi.mocked(findBinary).mockReturnValue(undefined)
		const tool = byId("hermes")
	...
	it("returns true when hermes binary is on PATH", () => {
		vi.mocked(findBinary).mockReturnValue("/usr/bin/hermes")
		const tool = byId("hermes")
		expect(tool?.isInstalled()).toBe(true)
	})

	it("returns false when neither directory nor binary is present", () => {
		vi.mocked(findBinary).mockReturnValue(undefined)
		const tool = byId("hermes")
		expect(tool?.isInstalled()).toBe(false)
	})
})
