	...
	if (command.includes("KIMCHI_HOOK_BLOCK") || /\bnpm\s+(install|i)\b/.test(command)) {
		emit({
			hookSpecificOutput: {
				...
				permissionDecisionReason: "codex: blocked by test hook",
			},
		})
	}
...
if (eventName === "PostToolUse" && (command.includes("KIMCHI_HOOK_POST_REWRITE") || toolOutput.includes("KIMCHI_HOOK_POST_REWRITE"))) {
	emit({
		hookSpecificOutput: {
			...
			updatedToolOutput: "codex: PostToolUse output rewrite OK",
		},
	})
}
...
if (eventName === "UserPromptSubmit") {
	if (prompt.includes("KIMCHI_HOOK_PROMPT_BLOCK")) {
		emit({
			hookSpecificOutput: {
				...
				permissionDecision: "deny",
				permissionDecisionReason: "codex: prompt blocked by test hook",
			},
		})
	}
	if (prompt.includes("KIMCHI_HOOK_PROMPT_REWRITE")) {
		emit({
			hookSpecificOutput: {
				hookEventName: "UserPromptSubmit",
				updatedInput: { prompt: prompt.replace("KIMCHI_HOOK_PROMPT_REWRITE", "codex rewrite OK.") },
			},
		})
	}
}
...
function writeLog(root, record) {
	...
	appendFileSync(join(dir, "codex-hook-adapter.log"), `${JSON.stringify(record)}\n`, "utf-8")
}
